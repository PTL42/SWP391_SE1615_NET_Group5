    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (ProductDTO f : cart) {
                if (f.getProductQuantity()> maxQuantity) {
                    f.setProductQuantity(txtProductID);
                    tmpQuantity = true;
                }
            }
            if (tmpQuantity) {
                request.setAttribute("CHECK_OUT_MSG", "Some products is out of stock, Max value input 1024."
                        + "Please check quantity again !!! This product not bigger than " + maxQuantity);
